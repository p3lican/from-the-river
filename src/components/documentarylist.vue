    ...
    <div v-else class="all-engaging-videos-wrapper">
     <div v-for="video in gs3Data" class="engaging-item">
       <div>
         ...
         <div v-else-if="video.gsx$videotype.$t=='youtube'" class="youtube-item">
         <div class="media-wrapper">

            <iframe width="560" height="315" :src="'https://www.youtube.com/embed/'+ video.gsx$youtubeurl.$t" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
export default {
  ...
  mounted() {
    ...
    this.loading = true;
    const self = this;
    const gs3 = 'https://spreadsheets.google.com/feeds/list/1MM4JKzk3m0xlZKf2UXAlb0Pdmk0zq0Cwo3xkz7YQa0o/4/public/values?alt=json';

    setTimeout(() => {
      axios.all([
        axios.get(gs3)
      ])
        .then(axios.spread((gs3Res) => {
          self.loading = false;
...
.facebook-item, .youtube-item, .instagram-item , .twitter-item, .igtv-item  {
    display: flex;
